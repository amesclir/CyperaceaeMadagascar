...
phy <- drop.tip(phy, c("Juncus_capitatus","Juncus_acutus","Juncus_effusus","Juncus_trifidus"))
...
## load data
mytree <- read.tree("./bamm/tree.tree")
mcmcout <- read.csv("./bamm/mcmc_out.txt")

## create edata
edata <- getEventData(mytree, eventdata = "./bamm/event_data.txt", burnin=0.25)
...
### Bayes factors
bfmat <- computeBayesFactors(postburn, expectedNumberOfShifts=1, burnin=0.25)
bfmat
...
plotRateThroughTime(edata, intervalCol="red", avgCol="red", start.time=st, ylim=c(0,0.6), cex.axis=1,ratetype='netdiv')
text(x=30, y= 0.5, label="Cyperaceae", font=4, cex=1.0, pos=4)

#other options
sc <- distinctShiftConfigurations(edata, expectedNumberOfShifts = 1, threshold = 5)
par(mfrow=c(1,1))
pdffn = paste0("bamm", ".coreshiftconfiguration1198_bis.pdf")
pdf(file=pdffn, width=8, height=18)
plot(sc, edata, index = 1198)
axisPhylo()
dev.off()
